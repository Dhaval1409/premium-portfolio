...
async function getProjectDetails(slug: string): Promise<Project | null> {
  try {
    ...
    const res = await fetch(`https://premium-portfolio-kohl.vercel.app/api/projects/${decodedSlug}`, {
      cache: 'no-store'
    });

    if (!res.ok) {
      console.error(`Backend returned status ${res.status} for project [${slug}]`);
      return null;
    }

    const data = await res.json();
    // Safely unwrap data if it's nested
...
export default async function ProjectCaseStudyPage({ params }: PageProps) {
  ...
  return (
    <article className="max-w-4xl mx-auto space-y-12 py-6 px-4">
      {/* Back to Work Nav Link */}
      <nav>
        <a 
          href="/projects" 
      ...
      {/* Main Project Cover Image Asset */}
      {project.thumbnail && (
        <div className="w-full aspect-[16/9] rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 shadow-sm bg-zinc-100 dark:bg-zinc-900">
          <img
            src={project.thumbnail}
      ...
      <footer className="pt-12 border-t border-zinc-200 dark:border-zinc-800 text-center">
        <p className="text-sm font-mono text-zinc-400">
          End of Specification • Created on {new Date(project.createdAt || new Date()).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
        </p>
      </footer>
    </article>
  );
