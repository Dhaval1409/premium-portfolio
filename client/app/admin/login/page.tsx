...
//     try {
//       // 🚨 FIXED: This safely pulls your Vercel URL instead of hardcoding localhost
...
export default function AdminLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // 🚨 FIXED: Hardcoded live backend URL to completely bypass Vercel environment variable bugs
      const res = await fetch('https://premium-portfolio-kohl.vercel.app/api/auth/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ password }),
      });
