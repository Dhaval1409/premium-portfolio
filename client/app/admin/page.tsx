...
//       setCustomImage(item.projectImage || '');
//       setGithubUrl(item.githubLink || '');
//     } else {
...
//           description, 
//           content, 
//           thumbnail: customImage || 'https://images.unsplash.com/photo-1555066931-4365d14bab8c', // 👈 Fixed from projectImage to thumbnail
//           githubUrl: githubUrl, // 👈 Fixed from githubLink to githubUrl
//           tags: cleanTags,
//           featured: true
//         }
//       : { 
...
//     try {
//       // Your endpoint route path handling logic checks out fine!
...
export default function AdminDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Fetch data streams from the contact API pipeline
      try {
        const messagesData = await fetchAPI('/contact');
        setMessages(Array.isArray(messagesData) ? messagesData : []);
      } catch (msgErr) {
        console.error('Failed to stream messaging logs:', msgErr);
      }
    } catch (err) {
      console.error('Error syncing backend database blocks:', err);
    } finally {
        ...
        <div className="flex items-center gap-4">
          <div className="flex bg-zinc-100 dark:bg-zinc-900 p-1 rounded-xl">
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-lg text-xs font-mono font-bold transition-all ${activeTab === 'messages' ? 'bg-white dark:bg-zinc-800 text-blue-500 shadow-xs' : 'text-zinc-400'}`}
            >
      ...
      {activeTab === 'messages' ? (
        <div className="border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden bg-white dark:bg-zinc-950/20">
          <div className="p-5 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50">
            <h2 className="text-xs font-mono font-bold text-zinc-400 uppercase tracking-widest">Incoming Data Inbound Transmissions</h2>
          </div>
          <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
            {loading ? (
              <div className="p-8 text-center font-mono text-zinc-400">Syncing message clusters...</div>
            ) : messages.length === 0 ? (
              <div className="p-8 text-center font-mono text-zinc-400">No communication signatures found in this database slot.</div>
            ) : (
              messages.map((msg: any) => (
                <div key={msg._id || Math.random().toString()} className="p-6 space-y-4 hover:bg-zinc-50/40 dark:hover:bg-zinc-900/5 transition-colors">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-dashed border-zinc-100 dark:border-zinc-900 pb-2">
                    <div>
                      <span className="font-mono text-xs font-bold text-zinc-400 uppercase tracking-wider">// FROM: </span>
                      <span className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">{msg.name}</span>
                      <span className="text-xs text-zinc-400 font-mono ml-2">&lt;{msg.email}&gt;</span>
                    </div>
                    {msg._id && (
                      <button 
                        onClick={() => handleDelete(msg._id)}
                        className="text-xs font-mono text-red-500 hover:underline text-left self-start sm:self-center"
                      >
                        [DROP TRANSMISSION]
                      </button>
                    )}
                  ...
                  <div>
                    <div className="text-xs font-mono text-zinc-400 uppercase tracking-wider mb-1">// OBJECTIVE</div>
                    <div className="text-sm font-medium text-zinc-800 dark:text-zinc-200">{msg.subject}</div>
                  </div>
                  <div>
                    <div className="text-xs font-mono text-zinc-400 uppercase tracking-wider mb-1">// PAYLOAD</div>
                    <p className="text-xs text-zinc-600 dark:text-zinc-400 bg-zinc-50 dark:bg-zinc-900/50 p-3.5 rounded-xl border border-zinc-100 dark:border-zinc-900/60 font-mono whitespace-pre-wrap leading-relaxed">
                      {msg.message}
                    </p>
                  </div>
                </div>
        ...
        /* Form rendering mechanics for Project / Editorial tab data blocks */
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <form onSubmit={handleSubmit} className="bg-zinc-50 dark:bg-zinc-900/40 p-6 border border-zinc-200 dark:border-zinc-800 rounded-2xl space-y-4 h-fit">
            <h2 className="text-xs font-mono font-bold text-zinc-400 uppercase tracking-widest">
              {isEditing ? '⚡ Update Entry' : '＋ Add Entry'} ({activeTab})
