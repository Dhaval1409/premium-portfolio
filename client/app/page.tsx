...
async function getFeaturedProjects(): Promise<Project[]> {
  try {
    // 🚨 We are directly using your exact, live backend URL here to bypass all Vercel cache/URL issues
    const res = await fetch('https://premium-portfolio-kohl.vercel.app/api/projects?featured=true', {
      cache: 'no-store'
    });

    if (!res.ok) {
      console.error('Backend rejected the request. Status:', res.status);
      return [];
    }

    const data = await res.json();
    
    // This safely extracts the array whether your Express API sends a direct array or an object
    ...
    console.error('Failed to fetch featured projects:', error);
    return []; // Fallback to avoid crashing the landing page
  }
}
