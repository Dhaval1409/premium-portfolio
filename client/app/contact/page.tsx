...
export default function ContactPage() {
  ...
  return (
    <div className="relative min-h-screen lg:h-screen bg-white dark:bg-zinc-950 text-zinc-950 dark:text-zinc-50 flex flex-col justify-between font-mono selection:bg-zinc-950 selection:text-white dark:selection:bg-white dark:selection:text-zinc-950 overflow-x-hidden">
      
      {/* Background Graphic Element - Design Sketch Asset */}
      <div className="absolute right-4 bottom-24 w-36 h-36 opacity-[0.07] dark:opacity-5 pointer-events-none mix-blend-multiply dark:invert hidden xl:block">
        <img 
          src="/Pin oleh talida di Desain grafis _ Sketsa, Perencana studi, Desain grafis.jpg" 
      ...
      <div className="w-full max-w-5xl mx-auto my-auto p-4 sm:p-6 lg:p-8">
        <div className="border border-zinc-200 dark:border-zinc-800 rounded-3xl overflow-hidden bg-white dark:bg-zinc-950 grid grid-cols-1 lg:grid-cols-12 shadow-2xs">
          ...
          <div className="lg:col-span-5 p-5 sm:p-8 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-zinc-200 dark:border-zinc-800 bg-zinc-50/30 dark:bg-zinc-900/10">
            ...
            {/* Cleaned up info grid with sensible labels */}
            <div className="grid grid-cols-2 gap-4 pt-6 border-t border-zinc-200/60 dark:border-zinc-800/60 text-[10px] tracking-widest text-zinc-400">
              <div>
          ...
          {/* Right Panel Compartment (Form Processing) */}
            ...
            <form onSubmit={handleSendMessage} className="space-y-4 flex flex-col">
              ...
              <div className="pt-2 self-end">
                <button 
                  ...
                  className="w-auto px-6 py-2.5 bg-zinc-950 text-white hover:bg-zinc-900 dark:bg-zinc-50 dark:text-zinc-950 dark:hover:bg-zinc-200 rounded-xl text-[10px] uppercase tracking-[0.25em] font-bold transition-all duration-300 transform active:scale-[0.98] disabled:bg-zinc-200 dark:disabled:bg-zinc-800 disabled:text-zinc-400 disabled:cursor-not-allowed cursor-pointer"
                >
                  {status === 'sending' ? (
                    <span>Sending...</span>
                  ) : status === 'success' ? (
