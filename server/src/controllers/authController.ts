...
export const loginAdmin = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    // FIXED: Explicitly handle environment variables as strings
    const masterPassword = (process.env.ADMIN_PASSWORD || 'fallbackpassword') as string;
    
    // Validate string equality
    if (password !== masterPassword) {
      res.status(401).json({ message: 'Invalid admin authentication signature' });
      return;
    }

    // FIXED: Explicitly cast secret to string for jwt.sign
